...
const App: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50">
      ...
      <nav className="bg-white border-t border-gray-200 pb-safe pt-2 px-2 shadow-lg z-20">
        <div className="flex justify-around items-center max-w-lg mx-auto">
          <TabButton active={mode === AppMode.TEXT} onClick={() => setMode(AppMode.TEXT)} icon={<Type size={20} />} label="Text" />
          <TabButton active={mode === AppMode.VOICE} onClick={() => setMode(AppMode.VOICE)} icon={<Mic size={20} />} label="Voice" />
          <button 
          ...
          <TabButton active={mode === AppMode.FILES} onClick={() => setMode(AppMode.FILES)} icon={<FileText size={20} />} label="Files" />
        </div>
      </nav>
    </div>
  );
